...
// =============================================================================
// Monotonicity: more fuel never changes a successful parse result
// =============================================================================
    ...
    decreases f, tokens.len() - idx, 0nat,
{
    ...
    // spec_parse_array_elements calls spec_parse_value with fuel-1
    lemma_parse_value_fuel_mono(input, tokens, idx, (f - 1) as nat, (f2 - 1) as nat);
    let result = spec_parse_value(input, tokens, idx, (f - 1) as nat);
    match result {
        None => {},
        Some((val, next)) => {
            let new_acc = acc + seq![val];
            if next >= tokens.len() {
                ...
                if next + 1 <= idx {
                    ...
                    // Sibling recursion uses same fuel, but position advances
